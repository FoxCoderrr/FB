...
app.controller('tab1Ctr', ['$interval', '$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($interval, $ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	...
	$httpService.httpFun({
		...
		$httpService.httpFun({
			...
			if($scope.my_contracts.length > 0) {
				$scope.myContract(0);
			} else {
	...
	$scope.interval_cancel1 = function() {
		if($scope.timer1) {
			$interval.cancel($scope.timer1);
		}
	};
	//国外定时请求
	$scope.outsideAjax = function(name) {
		$scope.interval_cancel();
		ajax_();
	...
	$scope.queryContract = function(i, n) {
		...
		$httpService.httpFun({
			...
			sessionStorage.setItem("activeshort", arr[0].code);
			sessionStorage.setItem("activebourse", arr[0].bourse_name);
			sessionStorage.setItem("activearr", JSON.stringify(arr));
			if(i == 1) {
				$scope.outsideAjax(n);
			} else {
	...
	$scope.myContract = function(x) {
		$httpService.httpFun({
			...
			$scope.arr = arr;
			console.log("我的自选：", $scope.arr);
			$scope.my_contracts = arr;
	...
	$scope.operate = function(e, i) {
		if($scope.my == 1) {
			if($scope.arr[0].area) {
				sessionStorage.setItem("activecontract", JSON.stringify([0, $scope.arr[i]]));
			} else {
				sessionStorage.setItem("activecontract", JSON.stringify($scope.arr[i]));
			}
		}

		var ar = JSON.parse(sessionStorage.getItem("activearr"));
		sessionStorage.setItem("activecontract", JSON.stringify(ar[i]));
		if($scope.t_id != "") {
			$scope.t_id = "";
		} else {
			sessionStorage.setItem("activecode", $scope.arr[i].code || $scope.arr[i].symbol);

			sessionStorage.setItem("activename", $scope.arr[i].name);
			sessionStorage.setItem("activeshort", $scope.arr[i].short || $scope.arr[i].symbol);
...
app.controller('edit_contractCtr', ['$interval', '$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($interval, $ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	...
	function sendAjax() {
		var list = $scope.my_con;
		var arr = [];
		for(let v of list) {
			v = v[0];
			if(v.change > 0) {
				arr.push([
					[v.name, v.close, v.change, v.changeRate, v.bid1Price, v.bid1Volume, v.ask1Price, v.ask1Volume, v.turnover, v.volume, "f_c_red", "false"]
				]);
			} else if(v.change < 0) {
				arr.push([
					[v.name, v.close, v.change, v.changeRate, v.bid1Price, v.bid1Volume, v.ask1Price, v.ask1Volume, v.turnover, v.volume, "f_c_green", "false"]
				]);
			} else {
				arr.push([
					[v.name, v.close, v.change, v.changeRate, v.bid1Price, v.bid1Volume, v.ask1Price, v.ask1Volume, v.turnover, v.volume, "f_c_white", "false"]
				]);
			}
		}
		$scope.my_contracts = arr;
	...
	//list点击
	$scope.operate = function(e, i, code) {
		if($scope.t_id != "") {
			$scope.t_id = "";
		} else {
			//选中
...
app.controller('tabdCtr', ['$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	sessionStorage.setItem("kperiod", "0");
	...
	$scope.doChange = function() {
		...
		var str = `<div class="k_bot_items">
					<div>
						<span><span data-period="005">5分钟</span></span>
						<span><span data-period="015">15分钟</span></span>
						<span><span data-period="030">30分钟</span></span>
						<span><span data-period="060">1小时</span></span>
						<span><span data-period="100">1天</span></span>
						<span><span data-period="200">1周</span></span>		
					</div>
				</div>`;
		$(".bot_modal").html(str);
		...
		if(sessionStorage.getItem("kperiod")) {
			$(".k_bot_items>div>span").eq(sessionStorage.getItem("kperiod")).find("span").addClass("s_select");
		} else {
		...
		$(".k_bot_items>div>span>span").on("click", function() {
			...
			sessionStorage.setItem("kperiod", $(this).parents("span").index());
			$(this).addClass("s_select");
			var type = $(this).data("period");

			var code = sessionStorage.getItem("activeshort");
			var arr = JSON.parse(sessionStorage.getItem("activecontract"));
			getKline(code, type);
			$(".k_bot_items").slideUp();
			$scope.active_change = false;
			$scope.$apply();
	...
	function getKline(code, type) {
		...
		if(arr.length != 1) {
			...
			if(type == 100 || type == 200) {
				url = `https://stock2.finance.sina.com.cn/futures/api/jsonp.php/var%20_RB1901=/InnerFuturesNewService.getDailyKLine?symbol=${code}`;
			} else {
				if(type == "001") {
					var type = 5;
				}
			...
			$.ajax({
				...
				success: function() {
					var res;
					if(type == 100 || type == 200) {
						var res = eval("_RB1901");
						if(type == 200) {
							var res_data_lines = [];
							for(var i = 0; i < res.length; i++) {
								var date = new Date(res[i].d); //转化成时间戳
								var t = Date.parse(date); //时间戳
								var o = Number(res[i].o); //开盘价
								var h = Number(res[i].h); //最高价
								var l = Number(res[i].l); //最低价
								var c = Number(res[i].c); //收盘价
								var v = Number(res[i].v); //成交量
								var arr = [t, o, h, l, c, v];
								res_data_lines.push(arr);
							}
							...
							for(var i = 0; i < res_data_lines.length; i++) {
								...
								week = Math.floor(Math.round((time - checkDate) / 86400000) / 7) + 1; //获取当前日期属于一年中的第几周
							...
							for(var i = 0; i < data_arr.length; i++) {
								...
								var o = data_arr[i][0][1]; //每周出事数据的开盘价 作为周开盘价
	...
	function build_all_data(res) {
		...
		for(var i = 0; i < res.length; i++) {
			...
			var arr = JSON.parse(sessionStorage.getItem("activecontract"));
			if(arr.length == 1) {
				res[i].date = res[i].date.toString();
				var time = res[i].date.substring(0, 4) + "-" + res[i].date.substring(4, 6) + "-" + res[i].date.substring(6, 8) + " " + res[i].time.substring(0, 2) + ":" + res[i].time.substring(2);
				var line_arr = [time, res[i].open, res[i].close, res[i].low, res[i].high, res[i].volume];
			} else {
				var time = res[i].d;
	...
	function build_kline(data) {
		...
		var option = {
			...
			dataZoom: [{
				type: 'inside',
				xAxisIndex: [0, 1, 2],
				startValue: data.times.length - 50,
				...
				itemStyle: {
					normal: {
						color: function(params) {
							...
							if(Number(data.datas[params.dataIndex][4]) > Number(data.datas[params.dataIndex - 1][4])) {
								colorList = '#ef232a';
							} else {
								colorList = '#2cfcf9';
							}
...
app.controller('tabaCtr', ['$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', 'dateSelect', function($ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory, dateSelect) {
	...
	$scope.addFun = function(i) {
		if(i) {
			...
			//时间条件单
			$httpService.httpFun({
				nozzle: "set_entrust_condition",
				token: sessionStorage.getItem("token"),
				code: sessionStorage.getItem("activecode"),
				direction: direction,
				mold: mold,
				price_type: 1,
				exceed_num: 0,
				number: $scope.left_hand,
				condition_type: 1,
				continuous_num: 0,
				continuous_eq: $scope.p_type,
				continuous_price: $scope.left_price,
				continuous_choice: 1,
				continuous_time: "",
				continuous_cancellations_sure: 1,
				continuous_cancellations: 0,
				continuous_cancellations_buy_sure: 1,
				continuous_cancellations_buy_num: 0,
				continuous_cancellations: 0,
				continuous_cancellations_buy: 1

			}).then(function(data) {
				console.log(data);
			});
			$("#top_info .info_con").html("添加成功");
			$("#top_info").fadeIn("fast", function() {
				setTimeout(function() {
					$("#top_info").hide();
				}, 1500)
			...
			$httpService.httpFun({
				...
				price_type: 1,
				exceed_num: 0,
				number: $scope.right_hand,
				condition_type: 1,
				continuous_num: 0,
				continuous_eq: $scope.p_type,
				continuous_price: $scope.right_price,
				continuous_choice: 1,
				continuous_time: $scope.time,
				continuous_cancellations_sure: 1,
				continuous_cancellations: 0,
				continuous_cancellations_buy_sure: 1,
				continuous_cancellations_buy_num: 0,
	...
	$scope.navTap1 = function(i) {
		$scope.list_id = i;
		if(i == 1) {
			$scope.c_list = $scope.c_list_l;
		} else {
			$scope.c_list = $scope.c_list_r;
		}
	}
	//条件单数据
	$httpService.httpFun({
		nozzle: "find_entrust_condition",
		token: sessionStorage.getItem("token")
	}).then(function(data) {
		console.log(data);
		...
		for(let v of data) {
			if(v.condition_type) {
				$scope.c_list_l.push(v);
			} else {
				$scope.c_list_r.push(v);
			}
		}
		$scope.navTap1(1);
	});
...
app.controller('tabbCtr', ['$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	...
	//			$scope.info.push([list[list.length - 1][6], list[list.length - 1][7], list[list.length - 1][4], list[list.length - 1][5], list[list.length - 1][1], list[list.length - 1][2], list[0], list[4], list[1], list[6], list[2], "--", list[9], list[10], "--", list[14], "--", list[13]]);
	//		} else {
	//			$scope.info.push([list[list.length - 1][6], list[list.length - 1][7], list[list.length - 1][4], list[list.length - 1][5], list[list.length - 1][1], list[list.length - 1][2], list[2], list[14], list[3], list[13], list[4], "--", "--", "--", "--", list[5], list[9], list[10]]);
	//		}
	//		//被替换内容(end)
	//	}else{
	//		var code = sessionStorage.getItem("activecode");
	//		$.ajax({
	//			cache: true,
	//			url: `http://47.110.136.149/snapshot`,
	//			type: 'post',
	//			dataType: 'jsonp',
	//			data:{
	//				symbol:code,
	...
	//			},
	//			success: function(res) {
	//				$scope.info.push([res[0].ask1Price,res[0].ask1Volume,res[0].bid1Price,res[0].bid1Volume,res[0].close,res[0].change,res[0].open,res[0].close,res[0].volume,res[0].high,res[0].turnover,res[0].low,"--",res[0].close,"--","--","--",res[0].pClose,"--","--"])
	//			}
	//		})
	//	}
...
app.controller('tabeCtr', ['$interval', '$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($interval, $ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	...
	//价格信息
	var act = JSON.parse(sessionStorage.getItem("activecontract"));
	$scope.price_info = act;
	...
	$scope.select = function(i, e) {
		...
		if(i == 1) {
			...
			$(".kb_con1>.span").on("click", function() {
				...
				$scope.top_name = $(this).html();
				$scope.active_name = $(this).html();
				$scope.$apply();
				$scope.reset();
			})
		} else if(i == 2) {
			...
			var str = `<div class="user_keyboard user_keyboard3">
						<div>
							<div class="top_head3">
								...
								<div class="ov_a">
									<span class="f_l">涨停：${minfo.up_limit}&nbsp;&nbsp;&nbsp;跌停：${minfo.down_limit}</span>
									<span class="f_r">最小变动：1</span>
								</div>
							</div>
							<div class="kb_con3 ov_a">
			...
			$(".kb_con3 .kb_devare").on("click", function() { //点击删除
				if($scope.leftc_price) {
					$scope.leftc_price_arr.pop();
					if($scope.leftc_price_arr[$scope.leftc_price_arr.length - 1] == ".") {
						$scope.leftc_price_arr.pop();
					}
					$scope.leftc_price = Number($scope.leftc_price_arr.join(""));
					console.log($scope.leftc_price);
			...
			$(".kb_con3 .kb_dot").on("click", function() {
				if($scope.leftc_price.toString().indexOf(".") == -1) {
					$scope.leftc_price_arr.push(".");
					$scope.leftc_price = $scope.leftc_price_arr.join("").toString();
					console.log($scope.leftc_price_arr, $scope.leftc_price);
					$scope.$apply();
				}
			})
...
app.controller('add_condition_orderCtr', ['$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', function($ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory) {
	...
	$scope.name = sessionStorage.getItem("activename");
	...
	$scope.addFun = function(i) {
		if(i) {
			...
			var codee = $scope.code || sessionStorage.getItem("activecode")
			//时间条件单
			$httpService.httpFun({
				nozzle: "set_entrust_condition",
				token: sessionStorage.getItem("token"),
...
app.controller('order_historyCtr', ['$ionicLoading', '$httpService', '$stateParams', '$rootScope', '$httpService', '$scope', '$http', '$state', '$ionicHistory', 'dateSelect', function($ionicLoading, $httpService, $stateParams, $rootScope, $httpService, $scope, $http, $state, $ionicHistory, dateSelect) {
	...
	$scope.search = function() {
		if($scope.inputData.startDate == "选择日期") {
			_toast("请选择日期");
			return false;
		}
		$httpService.httpFun({
			nozzle: "history_bill",
			token: sessionStorage.getItem("token"),
			time: $scope.inputData.startDate
		}).then(function(res) {
